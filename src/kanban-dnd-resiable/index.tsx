...
// Define a data structure for columns
const columnGroups = [
  {
    flex: 1,
    columns: [
  ...
  {
    flex: 2, // Middle column with two containers
    columns: [
...
const KanbanBoard: React.FC = () => {
  ...
  const renderColumn = (columnDef) => (
    <DroppableContainer id={columnDef.id} key={columnDef.id}>
      <div
        style={{
          width: '100%',
          flex: 1,
      ...
      >
        <h3 style={{ textAlign: 'center', color: '#f0f0f0' }}>{columnDef.title}</h3>
        <SortableContext
          items={columns[columnDef.id].items.map(item => item.id)}
          strategy={verticalListSortingStrategy}
        >
          <div style={{ minHeight: '50px' }}>
            {columns[columnDef.id].items.length > 0 ? (
              columns[columnDef.id].items.map(item => (
                isEditing === item.id ? (
      ...
      >
        {columnGroups.map((group, groupIndex) => (
          <div
            ...
            style={{
              display: 'flex',
              flexDirection: group.columns.length > 1 ? 'column' : 'column',
              flex: group.flex,
              gap: '16px',
            }}
          >
            {group.columns.map((columnDef) => renderColumn(columnDef))}
          </div>
        ))}
      </div>
      <DragOverlay>
